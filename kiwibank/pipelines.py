...
class KkiwibankPipeline:

    # Database setup
    conn = sqlite3.connect('kiwibank.db')
    c = conn.cursor()

    def open_spider(self, spider):
        self.c.execute("""CREATE TABLE IF NOT EXISTS `kiwibank`
                         (date text, title text, link text, content text)""")

    def process_item(self, item, spider):
        self.c.execute("""SELECT * FROM kiwibank WHERE title = ? AND date = ?""",
        ...
        # Insert values
        self.c.execute("INSERT INTO kiwibank (date, title, link, content)"
